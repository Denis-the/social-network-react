...
const UserItem = ({ user, followingInProgress, follow, unfollow, }) => {
    ...
    const followUnfollowButton = user.followed ?
        <ButtonUI styled={{width:'8rem'}} className="p-shadow-3"
        disabled={followingInProgress.has(user.id)} onClick={() => unfollow(user.id)}>Unfollow</ButtonUI>
        : <ButtonUI styled={{width:'8rem'}} className="p-shadow-3"
        disabled={followingInProgress.has(user.id)} onClick={() => follow(user.id)}>Follow</ButtonUI>
